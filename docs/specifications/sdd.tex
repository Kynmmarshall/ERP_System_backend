...
Identity & Users, tenants, Argon2 login, RS256 signing, refresh families & FastAPI; \code{identity_db}.\\
...
Identity institution metadata is not RLS-enabled. Bootstrap grants database ownership
...
An enrollment links a student UUID with program and term context.
...
on 80, database on 5432 and broker AMQP on 5672. Local gateway publishes 2022;
...
to the gateway on 2022; gateway-root \code{/healthz} is not covered by that Vite proxy.
...
Institution and user facts are separated in Identity. Academic separates
...
nullable user institution for platform use.
...
\end{itemize}
Cross-service user/student/program/enrollment IDs remain opaque UUIDs.
...
\diagram{06-erd-identity}{Identity entity relationship model}{Nullable tenant
...
Start with gateway \code{http://localhost:2022}; inspect each API's \code{readyz},
