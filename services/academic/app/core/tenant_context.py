...
async def set_platform_context(session: AsyncSession) -> None:
    """Used only by dev/seed scripts that must write rows across all
    tenants. Every request-handling endpoint must use set_tenant_context
    with the caller's real, verified claims instead.
    """
