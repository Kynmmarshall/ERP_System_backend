...
async def test_healthz_reports_ok_without_dependencies() -> None:
    ...
    assert response.json() == {"status": "ok", "service": "academic"}
...
async def test_readyz_reports_ready_when_database_reachable(monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "ready", "service": "academic"}
...
async def test_readyz_reports_unavailable_when_database_unreachable(monkeypatch) -> None:
    ...
    assert response.json() == {"status": "unavailable", "service": "academic", "reason": "database"}


# Unlike a purely unit-tested service, this suite requires a real reachable
# Postgres (see tests/test_enrollments.py) so there is no real-unreachable-
# database branch to exercise here without a mock; that branch is instead
# already covered by test_readyz_reports_unavailable_when_database_unreachable
# above.
