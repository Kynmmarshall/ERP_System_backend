...
async def test_finance_staff_cannot_create_a_course(client) -> None:
    """Teaching is the lecturer's alone - a finance or marketing account has
    no academic rights at all."""
    response = await client.post(
        "/api/v1/academic/courses",
        json={"code": "SEN101", "title": "Intro", "credits": 3},
...
async def test_marketing_cannot_create_a_course(client) -> None:
    response = await client.post(
        "/api/v1/academic/courses",
        json={"code": "SEN101", "title": "Intro", "credits": 3},
